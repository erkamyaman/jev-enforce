...
async function main() {
    ...
    if (cmd === 'rules') {
        ...
        if (!process.env.TYPESAFE_API_KEY) {
            for (const f of findRuleFiles(cwd, homedir())) {
                console.log(`\n${f}`);
                for (const r of extractCandidates(readFileSync(f, 'utf8')))
                    console.log(`  ?      ${r}`);
            }
    ...
    if (cmd === 'check') {
        ...
        if (kind !== 'reply' && kind !== 'code')
            throw new Error('--as must be reply or code');
        const file = args[0];
        ...
        for (const v of violations)
            console.log(`✗ ${v.p.toFixed(2)}  ${v.rule}`);

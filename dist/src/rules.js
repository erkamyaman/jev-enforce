...
export async function classify(candidates, ask, cacheDir) {
    ...
    if (missing.length) {
        ...
        const questions = Object.fromEntries(missing.map((_, i) => [`r${i}`, { type: 'choice', instructions: `What does rule r${i} constrain?`, criteria: SCOPES }]));
        const answers = await askAll(ask, state, questions);
        missing.forEach((t, i) => {
            const a = answers[`r${i}`];
            if (a?.type === 'choice' && isScope(a.choice))
                cache[hash(t)] = a.choice;
        });
...
export async function loadRules(cwd, home, ask, cacheDir) {
    const candidates = findRuleFiles(cwd, home).flatMap((source) => extractCandidates(readFileSync(source, 'utf8')).map((text) => ({ text, source })));
